...
**Question 1**: 
Research has shown that climate change is increasing the frequency of lightning strikes globally including in western North America [@perez2023]. This along with drier conditions increases the risk of potential large wildfires caused by lightning.
In particular, the 2024 Jasper wildfire is a strong example of a disastrous fire which was lightning caused. The Jasper wildfire resulted in mass evacuations, the destruction of houses and infrastructure, and serious environmental and economic damages in Alberta’s mountain parks. It is an example of how lightning caused wildfires are not only becoming more frequent but also becoming intense due to the climate change [@PublicSafety2025].
...
```{r, normcurve, warning=FALSE, echo=FALSE, fig.width=6, fig.height=4, fig.cap=bquote("Right-Tailed Test for Lightning-Caused Wildfire Proportions")}
#set up dataframe
...
## Question 2
Below, the distributions of the two variables used in the linear regression, fire spread rate and wind speed, are shown (Figure \@ref(fig:dist_fsr) ; \ref(fig:dist_ws)) The majority of observations tend to be near 0 for fire spread rate and between 0 to 25 for wind speed, with fewer observations at higher values.

```{r, dist_fsr, warning=F, echo=F, fig.width=6, fig.height=4, fig.cap="Distribution of Fire Spread Rate"}
...
```{r, dist_ws, warning=F, echo=F, fig.width=6, fig.height=4, fig.cap="Distribution of Wind Speed"}
...
Therefore, there is a 95% probability that $\beta_{1}$ is between `r paste(lb_b1, "and", ub_b1)` \@ref(CI_b1). Thus, we can reject the null hypothesis and confirm that $\beta_{1}$ is significantly different from 0. This, along with the p-value confirms that wind is a weak but significant contributor to fire spread rate.
...
```{r, residual_plot, echo=F, fig.width=6, fig.height=4, fig.cap="Residual Plot"}
...
```{r, norm_prob, echo=F, fig.width=6, fig.height=4, fig.cap="Normal Probability Plot of Standardized Residuals"}
s <- summary(lm_model)
SE = s$sigma
...
```{r, homosced, echo=FALSE, fig.width=6, fig.height=4, fig.cap="Homoscedasticity Check (Residuals vs Wind Speed)"}
